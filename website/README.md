...
served as a container behind the nginx container on `webapps-kz`.
...
docker save appmasterio/jc-rs-website:master-latest | gzip -1 \
  | ssh -p 442 os@10.10.3.33 'gunzip | docker load'
ssh -p 442 os@10.10.3.33 'cd /storage/jc-rs/website && docker compose up -d --force-recreate'
```

Then purge the Cloudflare cache, or the edge keeps serving the previous HTML:

```sh
source /home/os/sysadmin/secrets/cloudflare.env
curl -s -X POST -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
  -H "Content-Type: application/json" \
  "https://api.cloudflare.com/client/v4/zones/cfe25a3a86ec23e886d075a99deab437/purge_cache" \
...
| Thing | Value |
|---|---|
| Host | `webapps-kz`, `10.10.3.33`, SSH port 442, user `os` |
...
| DNS | `jc-rs.com` and `www.jc-rs.com`, proxied CNAME to the `webapps-kz` tunnel |
| Tunnel | `4c9de3b4-302b-4b40-b19f-09a7612c78df`, both hosts routed to `https://10.10.3.33:443` |
